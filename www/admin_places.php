...
$placeCategories = ["วัด/ศาสนา","ธรรมชาติ","ร้านอาหาร","แหล่งท่องเที่ยว","พิพิธภัณฑ์","ชุมชน","อื่นๆ"];
?>
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <div class="pg-card" id="places-list">
      ...
      <div class="place-group" data-search="<?= e(mb_strtolower($pl["name"] . " " . ($pl["category"] ?? "") . " " . ($pl["location_text"] ?? "") . " " . ($pl["district"] ?? "") . " " . ($pl["province"] ?? ""), "UTF-8")) ?>">
        ...
        <div id="<?= $rowId ?>" class="edit-panel" style="display:none">
          ...
          <form method="post" enctype="multipart/form-data" style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px">
