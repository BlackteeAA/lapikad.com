<?php
// Shared full-screen QR scanner page, used by scan.php and shop_scan_redeem.php.
// Caller must set before including this file:
//   $scannerTitle        - <title> text
//   $scannerBackHref     - back link URL
//   $scannerHint         - bottom hint text
//   $scannerFormAction   - form action URL
//   $scannerFormMethod   - "post" or "get"
//   $scannerFieldName    - hidden input name that receives the scanned code
//   $scannerHiddenFields - assoc array of extra hidden fields to render (name => value), optional
?>
<!DOCTYPE html>
<html lang="th">
<head>
...
<body>
  ...
  <!-- Top bar -->
  <div class="sc-top">
    <a href="<?= e($scannerBackHref) ?>" class="sc-back">
  ...
  <div class="sc-bottom">
    ...
    <div class="sc-hint"><?= e($scannerHint) ?></div>
  ...
  <form id="scan-form" method="<?= e($scannerFormMethod) ?>" action="<?= e($scannerFormAction) ?>" style="display:none">
    <?php foreach (($scannerHiddenFields ?? []) as $fieldName => $fieldValue): ?>
      <input type="hidden" name="<?= e($fieldName) ?>" value="<?= e($fieldValue) ?>">
    <?php endforeach; ?>
    <input type="hidden" name="<?= e($scannerFieldName) ?>" id="scanned_code">
  </form>
